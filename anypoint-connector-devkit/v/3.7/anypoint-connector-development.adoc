...
|link:/anypoint-connector-devkit/v/3.7/creating-a-connector-for-a-restful-api-using-jersey[RESTful API] a|Anypoint DevKit provides a way to build a Jersey-based client connector for RESTful APIs.

[NOTE]
`@RestCall` annotations have been *deprecated* for DevKit 3.8.0. Find the old `@RestCall` documentation in the DevKit 3.7 section link:/anypoint-connector-devkit/v/3.7/creating-a-connector-for-a-restful-api-using-restcall-annotations[Creating a Connector for a RESTful API using @RESTCall Annotations]
